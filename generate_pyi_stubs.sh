...
# Determine if --no-setup-py is supported
if pybind11-stubgen --help | grep -q -- '--no-setup-py'; then
    EXTRA_ARGS="--no-setup-py"
else
    EXTRA_ARGS=""
fi

# Run pybind11-stubgen
PYTHONPATH="$PP" pybind11-stubgen $EXTRA_ARGS -o stubs _nimblephysics \
    || { echo "pybind11-stubgen failed. Exiting."; exit 1; }
